...
class HandDetector:
    def __init__(
        self, mode=False, maxHands=2, detectionConfidence=0.5, trackConfidence=0.5
    ):
        """Store parameters for the hand detector"""
        self.mode = (
            mode  # If True, treats input as static images (not continuous video)
        )
        self.maxHands = maxHands  # Maximum number of hands to detect at once
        self.detectionConfidence = (
            detectionConfidence  # Minimum confidence for initial hand detection
        )
        self.trackConfidence = (
            trackConfidence  # Confidence threshold for tracking detected hands
        )
...
def main():
    ...
    while True:
        ...
        cv2.putText(
            img, str(int(fps)), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 0), 3
        )
